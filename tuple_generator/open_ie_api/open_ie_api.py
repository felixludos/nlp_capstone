...
from tuple_generator.open_ie_api.main import stanford_ie

tmp_folder = '/tmp/open_ie_api/'
...
def call_api_single(text, verbose=False):
    if os.path.isfile(text):
        full_tmp_filename = text
    else:
        full_tmp_filename = texts_to_files([text])[0]
    results = stanford_ie(full_tmp_filename, verbose=verbose)
    return results
